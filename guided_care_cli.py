...
def scan(store: CareStore, cancelled: bool) -> dict[str, Any]:
    ...
    if cancelled:
        cancel_event.set()
    findings, metrics, score, breakdown = Scanner(AppLogger()).run_full_scan(cancel_event=cancel_event)
    if cancel_event.is_set():
        store.append_event("scan_cancelled", {"status": "cancelled", "findings_count": len(findings)})
        return {"status": "cancelled", "findings": findings, "metrics": metrics, "health_score": score, "breakdown": breakdown}
...
def dispatch(args: argparse.Namespace) -> dict[str, Any]:
    if args.cancel and args.command != "scan":
        raise InputError("Invalid command input.")
    store = CareStore(args.state_dir)
